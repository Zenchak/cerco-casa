// Bootstrap dati case.
// Dopo il primo deploy di questa versione, l'elenco viene letto direttamente
// da GitHub (homes.json), così gli aggiornamenti agli annunci non richiedono
// più un nuovo deploy Netlify.
(function(){
  const fallback = [
    {
      id:'gerosa', name:'Bergamo · Via S. V. Gerosa', lat:45.6864, lng:9.6595,
      price:310000, type:'Bifamiliare', sqm:'191 m² commerciali', beds:2,
      garden:true, garage:false, city:true, top:false, status:'da-valutare', rating:'5,5/10',
      tags:['Giardino ~300 m²','2 camere','2 posti auto'],
      note:'Molto spazio e grande giardino, ma ferrovia raddoppiata praticamente sul confine. Niente vero garage.',
      warning:'Posizione pubblica approssimativa: l’annuncio non mostra il civico esatto.',
      url:'https://www.immobiliare.it/annunci/121237730/'
    },
    {
      id:'treviolo305', name:'Treviolo · villa di testa', lat:45.6738, lng:9.6138,
      price:305000, type:'Villa di testa', sqm:'162 m²', beds:3,
      garden:true, garage:true, city:false, top:true, status:'molto-interessante', rating:'9/10',
      tags:['3 camere','Giardino ~200 m²','Box doppio','3+ bagni','Annuncio attivo'],
      note:'Una delle proposte più centrate sui vostri criteri: tre camere, più bagni, giardino su tre lati e autorimessa doppia.',
      warning:'ANNUNCIO ATTIVO: verificato dalla pagina diretta dell’agenzia Eustacchio Immobiliare. Posizione indicativa sulla via.',
      url:'https://eustacchioimmobiliare.it/immobili/vendita/villa-di-testa/treviolo/41148.html'
    },
    {
      id:'treviolo315', name:'Treviolo · Viale Papa Giovanni XXIII 12', lat:45.6732, lng:9.6118,
      price:315000, type:'Villa di testa', sqm:'240 m²', beds:3,
      garden:true, garage:true, city:false, top:true, status:'molto-interessante', rating:'9/10?',
      tags:['3 camere','3 bagni','Giardino ~180 m²','Box doppio 40 m²','Taverna','1988'],
      note:'Proposta distinta dalla precedente: circa 240 m² su due livelli più interrato, 180 m² di giardino su tre lati, tre camere, tre bagni, taverna, cantina e box doppio. A 315.000 € è molto in linea con quello che cercate.',
      warning:'Annuncio attivo. Il civico è pubblicato; il pin è indicativo. Classe E, IPE 154,84 kWh/m²a.',
      url:'https://eustacchioimmobiliare.it/immobili/vendita/villa-di-testa/treviolo/39744.html'
    },
    {
      id:'seriate', name:'Seriate · Via Tagliamento', lat:45.6846, lng:9.7229,
      price:305000, type:'Villetta a schiera', sqm:'150 m²', beds:3,
      garden:true, garage:true, city:false, top:true, status:'molto-interessante', rating:'8,5/10',
      tags:['3 camere','2 giardini','Box doppio','3 bagni'],
      note:'Molto razionale per una famiglia: tre camere, tre bagni, due giardini, taverna e box doppio.',
      warning:'Posizione indicativa sulla via.',
      url:'https://www.immobiliare.it/annunci/132066786/'
    },
    {
      id:'azzano1', name:'Azzano San Paolo · Via Roma', lat:45.6570, lng:9.6745,
      price:320000, type:'Villa singola', sqm:'250 m²', beds:3,
      garden:true, garage:true, city:false, top:true, status:'molto-interessante', rating:'8,5/10?',
      tags:['3 camere','Casa singola','2 garage'],
      note:'Vera indipendenza e molta metratura. Prezzo basso per dimensioni: da capire bene il motivo.',
      warning:'Posizione indicativa sulla via.',
      url:'https://www.immobiliare.it/annunci/130669340/'
    },
    {
      id:'azzano2', name:'Azzano / Stezzano · Via Rossini', lat:45.6543, lng:9.6506,
      price:319000, type:'Villa singola', sqm:'198 m² commerciali', beds:3,
      garden:true, garage:true, city:false, top:false, status:'interessante', rating:'8/10',
      tags:['3 camere','Box doppio','Solaio ~100 m²'],
      note:'Libera su quattro lati, tre camere, giardino, box doppio e grande solaio al rustico. Un solo bagno oggi.',
      warning:'L’annuncio pubblica la zona in modo non perfettamente coerente.',
      url:'https://www.immobiliare.it/annunci/131749644/'
    },
    {
      id:'dalmine', name:'Dalmine · Mariano al Brembo', lat:45.6492, lng:9.6063,
      price:355000, type:'Villa di testa', sqm:'190 m²', beds:4,
      garden:true, garage:true, city:false, top:false, status:'interessante', rating:'7,5/10',
      tags:['4 camere','Giardino ~150 m²','Box doppio','3 bagni','Classe C','Annuncio attivo'],
      note:'Villa di testa libera su tre lati, edificata nel 2003, con quattro camere, giardino di circa 150 m², locale hobby/lavanderia e autorimessa doppia. Molto spazio già pronto per una famiglia, ma più lontana e quasi al tetto del budget.',
      warning:'ANNUNCIO ATTIVO: riferimento V006743. Casa.it risulta scaduto; il pulsante apre la copia attiva su TrovaCasa, verificata di recente. Posizione approssimativa a livello comunale; l’inserzionista non pubblica il civico esatto.',
      url:'https://www.trovacasa.it/annunci/bg-tc-5298-71646226'
    },
    {
      id:'valverde45', name:'Bergamo · Via Valverde 45', lat:45.712032, lng:9.659315,
      price:380000, type:'Terratetto / bifamiliare', sqm:'189 m²', beds:3,
      garden:true, garage:true, city:true, top:false, status:'da-valutare', rating:'7/10',
      tags:['3 camere','3 bagni','Giardino','Box + 3 posti auto','Segnalata da Camilla','Fuori parametri'],
      note:'Soluzione semindipendente in Valverde, ai piedi di Città Alta, con tre camere, tre bagni, spazio esterno e posti auto. Non rientra nei parametri economici attuali: richiesta €380.000, quindi circa €20.000 sopra il tetto massimo di €360.000.',
      warning:'FUORI PARAMETRI: prezzo superiore al budget massimo. Da considerare solo in caso di trattativa significativa. L’annuncio indica anche €167/mese di spese condominiali e classe energetica G.',
      url:'https://www.immobiliare.it/annunci/126966607/'
    }
  ];

  window.HOMES = fallback;

  try {
    const xhr = new XMLHttpRequest();
    const liveUrl = 'https://raw.githubusercontent.com/Zenchak/cerco-casa/main/homes.json?t=' + Date.now();
    xhr.open('GET', liveUrl, false);
    xhr.send(null);
    if (xhr.status >= 200 && xhr.status < 300) {
      const live = JSON.parse(xhr.responseText);
      if (Array.isArray(live)) window.HOMES = live;
    }
  } catch (err) {
    console.warn('Dati live GitHub non disponibili: uso la copia inclusa nel sito.', err);
  }
})();
